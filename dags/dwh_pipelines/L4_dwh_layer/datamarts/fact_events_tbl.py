...
def extract_data_from_dwh_l3() -> pd.DataFrame:
    """extract event's data from layer 3 prod

    Returns:
        pd.DataFrame: events_tbl_df
    """
    root_logger.info("Beginning extract_data_from_dwh_l3 process...")
    ...
    try:
        ...
        if postgres_connection is not None:
            ...
            # root_logger.debug("")
            root_logger.debug("Session connected to Postgres database closed.")
    root_logger.info("Ending extract_data_from_dwh_l3 process...")
...
def transform_data(df_extracted: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df_extracted (pd.DataFrame): data from dwh l3

    Returns:
        pd.DataFrame: transformed fact data
    """
...
def load_data_into_fact_tbl(df_transformed: pd.DataFrame) -> None:
    """Load transformed data into dwh l4

    Args:
        df_transformed (pd.DataFrame): transformed data from L3 ready to push into l4
    """
    root_logger.info("Beginning load_data_into_fact_tbl process...")
    try:
        postgres_connection, cursor = get_cursor(db_type, data_warehouse_layer)
        ...
        if postgres_connection is not None:
            ...
            # root_logger.debug("")
            root_logger.debug("Session connected to Postgres database closed.")
    root_logger.info("Ending load_data_into_fact_tbl process...")


def perform_fact_events_tbl() -> None:
    """Main process to input fact table L4"""
    enable_cross_database()
    df_extracted = extract_data_from_dwh_l3()
    df_transformed = transform_data(df_extracted)
    load_data_into_fact_tbl(df_transformed)
