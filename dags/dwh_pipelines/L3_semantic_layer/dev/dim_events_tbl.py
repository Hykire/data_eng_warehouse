...
(
    ...
    src_table_name="stg_events_tbl",
    table_name="dim_events_tbl",
...
def extract_data_from_dwh_l2() -> pd.DataFrame:
    """extract event's data from layer 2 prod

    Returns:
        pd.DataFrame: events_tbl_df
    """
    root_logger.info("Beginning extract_data_from_dwh_l2 process...")
    ...
    try:
        ...
        if postgres_connection is not None:
            ...
            # root_logger.debug("")
            root_logger.debug("Session connected to Postgres database closed.")
    root_logger.info("Ending extract_data_from_dwh_l2 process...")
    return events_tbl_df


def transform_data_with_business_rules(df_extracted: pd.DataFrame) -> pd.DataFrame:
    """Convert the business rules into code logic to reflect the true state of business events

    Args:
        df_extracted (pd.DataFrame): data from dwh l2

    Returns:
        pd.DataFrame: transformed data with business rules
    """
    root_logger.info("Beginning transform_data_with_business_rules process...")
    # perform business rules logic
    df_extracted = df_extracted[
        (df_extracted["t"] >= "2013-01-01") & (df_extracted["t"] <= "2023-12-31")
    ]
    df_extracted = df_extracted[
        (df_extracted["t_event"] >= "2013-01-01")
        & (df_extracted["t_event"] <= "2023-12-31")
    ]
    df_extracted["dwh_layer"] = data_warehouse_layer
    df_extracted = df_extracted.replace({np.NaN: None})
    root_logger.info("Ending transform_data_with_business_rules process...")
    return df_extracted


def load_data_into_dim_tbl(df_transformed: pd.DataFrame) -> None:
    """Load transformed data into dwh l3

    Args:
        df_transformed (pd.DataFrame): transformed data from L2 ready to push into l3
    """
    root_logger.info("Beginning load_data_into_dim_tbl process...")
    try:
        postgres_connection, cursor = get_cursor(db_type, data_warehouse_layer)
        # Set up SQL statements for table creation and validation check
        create_stg_events_tbl = f"""CREATE TABLE IF NOT EXISTS {active_schema_name}.{table_name} (
                                        events_sk               SERIAL PRIMARY KEY,
                                        event                   varchar(255),
        ...
        if postgres_connection is not None:
            ...
            # root_logger.debug("")
            root_logger.debug("Session connected to Postgres database closed.")
    root_logger.info("Ending load_data_into_dim_tbl process...")


def main_dwh_l3_dev() -> None:
    """Main process to get data into dwh l3"""
    enable_cross_database()
    df_extracted = extract_data_from_dwh_l2()
    df_transformed = transform_data_with_business_rules(df_extracted)
    load_data_into_dim_tbl(df_transformed)
